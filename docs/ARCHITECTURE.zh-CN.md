...
> [English](ARCHITECTURE.md) | 简体中文 · 快照 `2026-08-03T23:12:23+08:00`

## 设计目标

1. 数据与镜像准备完成后可以长期断网运行。
2. 个人点位、轨迹、备注和媒体始终由本地用户控制。
3. 江苏/安徽区域单元能够复制到更多省份、国家和地区。
...
区域包只有在 PMTiles 与 manifest 同时通过校验后才算独立安装；否则处于可获取、暂存或部分安装状态。本快照中通过校验并已安装的是江苏和安徽。

OSM Carto 是当前默认的熟悉地图视觉。它使用独立数据库和瓦片缓存，构建过程可续建，在新渲染器健康前不替换现有版本。PMTiles 继续提供可交互的矢量地图、点击属性和独立区域生命周期。
...
参考索引和 Nominatim 都是可再生产品，不是个人权威数据。当前共享范围是江苏和安徽。
