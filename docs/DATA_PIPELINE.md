...
- catalogued `.poly` boundaries. Jiangsu/Anhui and Shanghai/Zhejiang are installed now.
...
The generated archive must be larger than 10MB and begin with the seven-byte `PMTiles` signature. SHA256 is printed after replacement. A machine-readable manifest is written beside each archive with province identity, source timestamp/sequence, bounds, sizes, and hashes. The current products are approximately 412.7 MiB for Jiangsu, 329.5 MiB for Anhui, 93.7 MiB for Shanghai, and 460.6 MiB for Zhejiang.
